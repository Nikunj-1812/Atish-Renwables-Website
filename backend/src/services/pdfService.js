...
function generateSolarReportPdf(data, stream) {
  ...
  doc.fillColor(TEXT_COLOR).font('Helvetica').fontSize(8.5);
  doc.text('Estimated 30-Year Cumulative Generation', 60, row4Y + 6);
  ...
  doc.text('Total Project Cost', 60, fRow1Y + 6);
  doc.font('Helvetica-Bold').fillColor(PRIMARY_COLOR).text(formatINR(data.netCost), 320, fRow1Y + 6);

  const fRow2Y = finTableTop + 40;
  doc.rect(50, fRow2Y, 495, 20).fill(BG_LIGHT);
  doc.fillColor(TEXT_COLOR).font('Helvetica').fontSize(8.5);
  doc.text('Yearly Electricity Bill Savings', 60, fRow2Y + 6);
  doc.font('Helvetica-Bold').fillColor(TEXT_COLOR).text(formatINR(data.yearlySavings), 320, fRow2Y + 6);

  const fRow3Y = finTableTop + 60;
  doc.rect(50, fRow3Y, 495, 20).fill('#ffffff');
  doc.fillColor(TEXT_COLOR).font('Helvetica').fontSize(8.5);
  doc.text('Estimated Payback Period', 60, fRow3Y + 6);
  doc.font('Helvetica-Bold').text(`${data.paybackPeriodYears.toFixed(1)} Years`, 320, fRow3Y + 6);

  const fRow4Y = finTableTop + 80;
  doc.rect(50, fRow4Y, 495, 20).fill(BG_LIGHT);
  doc.fillColor(TEXT_COLOR).font('Helvetica').fontSize(8.5);
  doc.text('System Return on Investment (ROI)', 60, fRow4Y + 6);
  doc.font('Helvetica-Bold').text(`${data.roi.toFixed(1)}%`, 320, fRow4Y + 6);

  const fRow5Y = finTableTop + 100;
  doc.rect(50, fRow5Y, 495, 20).fill('#ffffff');
  doc.fillColor(TEXT_COLOR).font('Helvetica-Bold').fontSize(8.5);
  doc.text('30-Year Cumulative Financial Savings', 60, fRow5Y + 6);
  doc.font('Helvetica-Bold').fillColor(PRIMARY_COLOR).text(formatINR(data.lifetimeSavings), 320, fRow5Y + 6);

  doc.rect(50, finTableTop, 495, 120).strokeColor(BORDER_COLOR).lineWidth(1).stroke();
  ...
  doc.fillColor(PRIMARY_COLOR).font('Helvetica-Bold').fontSize(11).text('3. RETURN ON INVESTMENT PROJECTION (30 YEARS)', 50, 75);
  ...
  doc.text('Year 6', chartX + chartW * 0.2 - 10, chartY + chartH + 10);
  doc.text('Year 12', chartX + chartW * 0.4 - 10, chartY + chartH + 10);
  doc.text('Year 18', chartX + chartW * 0.6 - 10, chartY + chartH + 10);
  doc.text('Year 24', chartX + chartW * 0.8 - 10, chartY + chartH + 10);
  doc.text('Year 30', chartX + chartW - 10, chartY + chartH + 10);
  ...
  doc.fillColor(TEXT_COLOR).font('Helvetica').fontSize(8.2).text(
    ...
    '3. To proceed with the engineering review and obtain a custom commercial quotation, please schedule a free physical site survey.',
    50, 455, { lineGap: 3.5 }
  );
